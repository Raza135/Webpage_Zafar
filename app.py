...
def index():
    # global data
    # data = {}
    session["session_id"] = os.urandom(2)
...
def getMessage():
    ...
    db.execute("INSERT INTO message (time, name, message, email) VALUES (:timeI, :nameI, :messageI, :emailI)", {
               "timeI": timestamp, "nameI": name, "messageI": message, "emailI": email})
...
def getKeyWords():
    ...
    session['numParas'] = [i for i in range(1, int(numParas) + 1)]

    timestampKeywords = datetime.datetime.now()

    session['timestampKeywords'] = timestampKeywords

    db.execute("INSERT INTO keywords (key1, key2, key3, key4, key5, time) VALUES (:key1I, :key2I, :key3I, :key4I, :key5I, :timeI)", {
               "key1I": key1, "key2I": key2, "key3I": key3, "key4I": key4, "key5I": key5, "timeI": timestampKeywords})

    db.commit()

    keywordsID = list(db.execute("SELECT idkeywords FROM keywords WHERE key1 = :key1S AND key2 = :key2S AND key3 = :key3S AND key4 = :key4S AND key5 = :key5S AND time = :timeS", {'key1S': key1, 'key2S': key2, 'key3S': key3, 'key4S': key4, 'key5S': key5, 'timeS': timestampKeywords}))[0][0]

    session['keywordsID'] = keywordsID

    print("keywordID:", keywordsID)
...
def paragraphs():
    ...
    for i in session['paras']:
        db.execute("INSERT INTO paragraph (keywords_idkeywords, para) VALUES (:keywordIDI, :paraI)", {
                   "keywordIDI": keywordsID, "paraI": i})

    db.commit()

    # print(session)

    return render_template("paragraphs.html", data=session)
...
def getFeedback():
    ...
    for i in range(1, len(session['numParas']) + 1):
        ...
        feedback['para'+str(i)] = {'c': coherency,
                                   'r': relevance, 'g': grammar}

        paraID = list(db.execute("SELECT idparagraph FROM paragraph WHERE keywords_idkeywords = :keywordIDS AND para = :paraS", {
                      'keywordIDS': session['keywordsID'], 'paraS': session['paras'][i-1]}))[0][0]
        
        print(paraID)

        db.execute("INSERT INTO feedback (paragraph_idparagraph, coherency, relevance, grammar) VALUES (:paraIDI, :coherencyI, :relevanceI, :grammarI)", {
                   "paraIDI": paraID, "coherencyI": coherency, "relevanceI": relevance, "grammarI": grammar})
...
if __name__ == '__main__':
    ...
    word_vec = np.concatenate(
        (np.array([[0]*word_vec.shape[1]] * 4), word_vec))
    ...
    def beam_search(topics, num_chars, model, idx_to_word, word_to_idx, is_sample=False):
        ...
        log_prob, output, hidden, attn_weight, coverage_vector = model.inference(
            inputs=X, topics=topics, output=output, hidden=hidden, coverage_vector=coverage_vector, seq_length=seq_length)
        log_prob = log_prob.cpu().detach().reshape(-1).numpy()
    #     print(log_prob[10])
        """2"""
        if is_sample:
            top_indices = np.random.choice(
                vocab_size, beam_size, replace=False, p=np.exp(log_prob))
        else:
            top_indices = np.argsort(-log_prob)
        """3"""
        beams = [(0.0, [idx_to_word[1]], idx_to_word[1], torch.zeros(
            1, topics.shape[1]), torch.ones(1, topics.shape[1]))]
        b = beams[0]
        beam_candidates = []
    ...
    #     assert False
        for i in range(beam_size):
            ...
            beam_candidates.append((b[0]+log_prob[word_idx], b[1]+[idx_to_word[word_idx]], word_idx, torch.cat(
                (b[3], attn_weight[0].cpu().data), 0), torch.cat((b[4], coverage_vector.cpu().data), 0), hidden, output.squeeze(0), coverage_vector))
        """4"""
        beam_candidates.sort(
            key=lambda x: x[0], reverse=True)  # decreasing order
        ...
        for xy in range(num_chars-1):
            ...
            for b in beams:
                ...
                """6"""
                log_prob, output, hidden, attn_weight, coverage_vector = model.inference(
                    inputs=X, topics=topics, output=b[6], hidden=b[5], coverage_vector=b[7], seq_length=seq_length)
                log_prob = log_prob.cpu().detach().reshape(-1).numpy()
                """8"""
                if is_sample:
                    top_indices = np.random.choice(
    ...
    def evaluateAndShowAttention(input_sentence, method='beam_search', is_sample=False):
        ...
        if method == 'beam_search':
            _, output_words, attentions, coverage_vector = beam_search(
                input_sentence, num_chars, model, idx_to_word, word_to_idx, is_sample=is_sample)
        else:
            _, output_words, attentions, _ = predict_rnn(
                input_sentence, num_chars, model, idx_to_word, word_to_idx)
    ...
    adaptive_softmax = nn.AdaptiveLogSoftmaxWithLoss(hidden_dim, len(
        vocab), cutoffs=[round(vocab_size / 20), 4*round(vocab_size / 20)])
    ...
    class Attention(nn.Module):
        ...
        def forward(self, query, topics, coverage_vector):
            ...
            for i in range(topics.shape[1]):
                ...
                query = self.Wa(query)
                scores += [self.va(torch.tanh(query + proj_key))
                           * C_t[:, i:i+1]]
    ...
    class AttentionDecoder(nn.Module):
        ...
        def forward(self, input, output, hidden, phi, topics, coverage_vector):
            ...
            mt, score = self.attention(
                output.squeeze(0), topics, coverage_vector)
    ...
    class MTALSTM(nn.Module):
        def __init__(self, hidden_dim, embed_dim, num_keywords, num_layers, weight,
            ...
            self.Uf = nn.Linear(embed_dim * num_keywords,
                                num_keywords, bias=False)
        ...
        def forward(self, inputs, topics, output, hidden=None, mask=None, target=None, coverage_vector=None, seq_length=None):
            ...
            for i in range(len(inputs)):
                output, hidden, score, coverage_vector = self.decoder(input=inputs[i].unsqueeze(
                    0), output=output, hidden=hidden, phi=phi, topics=topics_embed, coverage_vector=coverage_vector)  # [seq_len x batch x embed_size]
            ...
            outputs = self.adaptiveSoftmax(
                output_states.reshape(-1, output_states.shape[-1]), target.t().reshape((-1,)))
        ...
        def inference(self, inputs, topics, output, hidden=None, mask=None, coverage_vector=None, seq_length=None):
            ...
            phi = None
            phi = seq_length.float() * \
                torch.sigmoid(self.Uf(topics_embed.reshape(
                    topics_embed.shape[0], -1).float()))
    ...
    lr_scheduler = ReduceLROnPlateau(
        optimizer, mode='min', factor=0.4, patience=2, min_lr=1e-7, verbose=True)
    ...
    if True:
        ...
        model.load_state_dict(torch.load(
            model_check_point, map_location='cpu'))
        model = model.to(device)
        optimizer.load_state_dict(torch.load(
            optim_check_point, map_location='cpu'))
        lr_scheduler = ReduceLROnPlateau(
            optimizer, mode='min', factor=0.4, patience=2, min_lr=1e-7, verbose=True)
